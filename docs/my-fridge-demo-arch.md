...
4. 用户也可以在今日页手动选择已有菜谱加入某一餐。
5. 生成或手动加入的结果都只是计划，不会立即扣减库存。
6. 用户确认做了某个餐项后，系统先复查库存，库存足够才扣减。
7. 用户取消确认后，系统恢复之前扣减的库存。
8. 用户可以导出和导入本地数据，方便备份和换设备。
...
今日页新增 `RecipePicker` 类组件或等价组件，用于手动选择菜谱。它只负责搜索、筛选和触发选择，不直接修改库存。
...
随机生成和手动添加菜谱都应使用同一套 helper 创建 `PlannedMealItem`，避免出现两套不同的库存消耗计算。

手动添加库存不足的菜谱时，餐项仍可进入计划，但 `warnings` 必须记录缺食材、数量不足或单位需要确认等信息。该 warning 不阻止计划保存，但会阻止后续确认扣减。
...
1. 用户在今日页的早餐、午餐或晚餐卡片点击“添加菜谱”。
2. UI 打开菜谱选择器。
3. 用户可以搜索菜谱名称。
4. 用户可以按餐次筛选，默认使用当前餐次。
5. 用户可以按营养标签筛选。标签展示顺序为：`carb`、`protein`、`fat` 在前，`fiber`、`vegetable`、`fruit`、`dairy` 在后。
6. 系统为列表中的菜谱计算库存可用性。
7. 库存足够的菜谱正常显示。
8. 库存不足、数量不足或单位无法自动确认的菜谱仍可显示并允许加入，但必须显示 warning。
9. 用户选择菜谱后，系统把该菜谱转换成 `PlannedMealItem`，追加到当前餐次的 `items`。
10. 手动添加只保存计划，不扣减库存。

手动添加使用与随机生成相同的 `plannedConsumption` 计算规则。不同点是：随机生成会过滤不可行候选；手动添加允许不可行候选进入计划，但必须保留 warning。

### 6.5 确认餐项

1. 用户点击某餐项的确认。
2. 系统根据最新库存复查 `plannedConsumption`。
3. 如果库存不足、食材已删除或单位需要确认，系统不扣减库存，并显示错误提示。
4. 如果库存足够，系统根据 `plannedConsumption` 扣减库存。
5. 系统写入 `InventoryTransaction`。
6. 餐项状态改为 `completed`。
7. 餐项锁定，不再被随机覆盖。
...
手动添加菜谱不使用同样的硬过滤。手动添加时：

- 餐次不匹配的菜谱默认不展示，但用户可通过筛选改成其他餐次或全部餐次。
- 缺食材、数量不足或单位无法自动确认的菜谱仍可加入计划。
- 不足信息必须写入 `warnings` 并在 UI 中显示。
- 真正点击确认扣减时必须再次校验，校验失败则不写库存和交易记录。
...
手动选择器运行在 Today 页，不属于全局菜谱管理页。

筛选维度：

- 搜索词：匹配菜谱名称。
- 餐次：默认当前餐次，可切换到早餐、午餐、晚餐或全部。
- 营养标签：支持多选。
...
这个顺序体现 Demo 的轻量营养重点：先让用户快速判断主食、蛋白质和脂肪，再查看其他补充标签。
...
手动添加菜谱时，如果发现问题，允许加入计划，但显示 warning：

- 缺少食材
- 数量不足
- 单位需要确认

用户确认餐项时，如果问题仍存在，系统必须阻止确认，不扣库存，不写 `InventoryTransaction`。
...
- 库存数量不足
- 食材已被删除
- 单位无法自动扣减，需要用户先调整库存或菜谱单位

失败时保留餐项为 `planned`，不改变库存。
...
- 单位不一致时标记需要确认
- 已确认餐项不会被随机覆盖
- 手动添加菜谱会追加到当前餐次，而不是覆盖已有餐项
- 手动添加库存不足的菜谱会保留 warning
- 库存不足的餐项确认时不能扣减库存
- 手动菜谱选择器的餐次筛选、营养标签筛选和搜索
- 取消确认能恢复库存
- 同日尽量避免重复
